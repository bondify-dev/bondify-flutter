...
/// Drop-in кнопка "Войти через Telegram".
/// Автоматически запускает сессию и открывает Telegram.
///
/// ```dart
/// BondifyButton(
///   onSuccess: (user) => print('Привет, ${user.telegramName}!'),
/// )
/// ```
    ...
    return ElevatedButton(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            isLoading   ? 'Создание сессии…'           :
            isConfirmed ? '✅ Вход выполнен'            :
                          widget.label,
...
/// Нижний drawer с QR-кодом и статусом авторизации.
///
/// ```dart
  ...
  Widget _buildContent() {
    switch (_client.status) {
      case BondifyAuthStatus.pending:
        return const _LoadingState(message: 'Создание сессии…');
      ...
      case BondifyAuthStatus.idle:
        return const _LoadingState(message: 'Инициализация…');
    }
  }
}
...
/// Реактивный builder, перестраивается при изменении статуса авторизации.
///
/// ```dart
/// BondifyStatusBuilder(
...
class _PollingState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        // Placeholder QR (для реального QR используйте пакет qr_flutter)
        Container(
        ...
        if (secondsLeft != null)
          Text(
            'Ссылка действительна ещё ${_formatSeconds(secondsLeft!)}',
  ...
  static String _formatSeconds(int s) {
    final m = s ~/ 60;
    final r = s % 60;
    return m > 0 ? '$m:${r.toString().padLeft(2, '0')}' : '${s}с';
  }
}
...
class _ErrorState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final emoji  = isCancelled ? '🚫' : isExpired ? '⏰' : '⚠️';
